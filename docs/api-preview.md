...
Radio IP changes accept only public IPv4 addresses; private and reserved ranges are rejected. Active Radio changes are remote-first, so an uncertain timeout, local commit failure, or concurrent local change returns `503 radio_ip_reconciliation_required` with `reconciliation_required=true` instead of inviting an unsafe retry. Radio endpoint lists publish only canonical HTTPS URLs and reject credential-bearing URLs such as `https://user:pass@host/...`. An owned JTS3 service with no valid Zone mapping returns `503 service_zone_unavailable` rather than an indistinguishable empty catalog.
