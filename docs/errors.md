...
The current TeaSpeak/Radio extension operations use the durable pending-claim rule: completed results are replayed for 24 hours, but an unresolved pending claim is not automatically expired or re-executed after an uncertain side effect. The current contract also uses `503 authentication_unavailable` for redacted authentication-backend failures and `503 radio_ip_reconciliation_required` with `reconciliation_required=true` when a remote-first Radio IP mutation has an uncertain outcome.
